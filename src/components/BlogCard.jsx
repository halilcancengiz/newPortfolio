...
const BlogCard = ({ post, index }) => {
  ...
  const { postId, postTitle, content, metaDescription, createdAt, readingCount } = post;
  ...
  return (
    <>
      {post && (
        <div className={`bg-black-400 overflow-hidden flex rounded-lg items-center justify-center mb-10 max-w-[1000px] mx-auto md:flex-row xs:flex-col `}>
          <div id="last-posts" className="overflow-hidden xs:w-full md:max-w-[50%] w-full flex items-center justify-center" >
            <Image className="aspect-[4/2]  p-5 object-cover drop-shadow-green-btn transition-all bg-center duration-500" src={image} alt="" />
          </div>
                ...
                <div className="flex items-center justify-between my-1">
                  <Tooltip title="Tahmini Okuma Süresi">
                    <div className="flex items-center my-1">
                      <AiOutlineClockCircle className="text-blue-btn mr-2" size={16} />
                      <span className="text-xs italic">{calculateReadingTime(content)}</span>
                    </div>
                  </Tooltip>
                  <Tooltip title="Görüntülenme">
                    <div className="flex items-center my-1">
                      <AiFillEye className="text-blue-btn mr-2" size={16} />
                      <span className="text-xs italic">{readingCount}</span>
                    </div>
                  </Tooltip>
                  <Tooltip title="Yorum">
                    <div className="flex items-center my-1">
